/**
 * Ekonomi Geçmişi Seed Script
 * 17 ülke için 2018-2023 dönemi verilerini ekler
 */
...
const seedData = async () => {
  console.log('🌱 Ekonomi Geçmişi Seed başlatılıyor...\n');
  ...
  try {
    ...
    `);
    console.log('✅ Tablo oluşturuldu/kontrol edildi');

    // Mevcut verileri temizle
    await pool.query('DELETE FROM ekonomi_gecmis');
    console.log('🗑️ Mevcut veriler temizlendi');

    // Veriler - 17 ülke, 6 yıl
    const data = [
      // TÜRKİYE (1)
    ...
    // Toplu insert
    const insertQuery = `
      INSERT INTO ekonomi_gecmis 
      (ulke_id, yil, toplam_gsyh_milyar_dolar, gsyh_kisi_basi_usd, buyume_orani_yuzde, 
       enflasyon_orani_yuzde, issizlik_orani_yuzde, toplam_ihracat_milyar_dolar, 
       toplam_ithalat_milyar_dolar, nufus_milyon)
      VALUES ?
    `;

    await pool.query(insertQuery, [data]);
    console.log(`✅ ${data.length} kayıt eklendi`);

    // Kontrol
    const [result] = await pool.query('SELECT COUNT(*) as total FROM ekonomi_gecmis');
    console.log(`\n📊 Toplam kayıt: ${result[0].total}`);

    // Ülke bazında kontrol
    const [ulkeler] = await pool.query(`
      SELECT u.ulke_adi, COUNT(eg.id) as kayit_sayisi 
      FROM ekonomi_gecmis eg 
      JOIN ulkeler u ON eg.ulke_id = u.ulke_id 
      GROUP BY eg.ulke_id, u.ulke_adi 
      ORDER BY u.ulke_adi
    `);
    
    console.log('\n📋 Ülke bazında kayıtlar:');
    ulkeler.forEach(u => console.log(`   - ${u.ulke_adi}: ${u.kayit_sayisi} yıl`));
